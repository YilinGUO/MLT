...
def getRSI(symbol=CONSTANTS.DEFAULT_SYMBOL,
	...
	dummy_start_date = sd - dt.timedelta(CONSTANTS.LOOKBACK_RSI * 2)
	...
	for index, current_date in enumerate(date_list):
		if index < CONSTANTS.LOOKBACK_RSI:
			continue

		average_gain = 0.0
		average_loss = 0.0

		for i in range(CONSTANTS.LOOKBACK_RSI):
			current_day_value = prices.ix[date_list[index - CONSTANTS.LOOKBACK_RSI + i + 1], symbol]
			prev_day_value = prices.ix[date_list[index - CONSTANTS.LOOKBACK_RSI + i], symbol]
		...
		average_loss /= CONSTANTS.LOOKBACK_RSI
		average_gain /= CONSTANTS.LOOKBACK_RSI
		...
		if abs(average_loss-0) < CONSTANTS.EPSILON:
			...
			rsi.ix[current_date, CONSTANTS.RSI] = 100.0 - 100.0 / (1 + rsi.ix[current_date, CONSTANTS.RS])
...
def getPriceSMARatio(symbol=CONSTANTS.DEFAULT_SYMBOL,
					 sd=CONSTANTS.IN_SAMPLE_START, ed=CONSTANTS.IN_SAMPLE_END):
	dummy_start_date = sd - dt.timedelta(CONSTANTS.LOOKBACK_PRICE_SMA * 2)
	prices = getResolvedAdjustedPrices([symbol], pd.date_range(dummy_start_date, ed))
	...
	for index, current_date in enumerate(date_list):
		if index < CONSTANTS.LOOKBACK_PRICE_SMA:
			continue

		assert date_list[index] == current_date
		price_sma_df.ix[current_date, CONSTANTS.SMA] = \
			price_sma_df.ix[date_list[index - CONSTANTS.LOOKBACK_PRICE_SMA + 1]:date_list[index],
...
def getWilliamR(symbol=CONSTANTS.DEFAULT_SYMBOL,
				sd=CONSTANTS.IN_SAMPLE_START, ed=CONSTANTS.IN_SAMPLE_END):
	dummy_start_date = sd - dt.timedelta(CONSTANTS.LOOKBACK_WILLIAMS * 2)
	prices = getResolvedAdjustedPrices([symbol], pd.date_range(dummy_start_date, ed))
	...
	date_list = prices.index.values
	for index, current_date in enumerate(date_list):
		if index < CONSTANTS.LOOKBACK_WILLIAMS + 2:
			continue

		assert date_list[index] == current_date
		william_r_df.ix[current_date, CONSTANTS.HIGHEST_HIGH] = \
			prices.ix[date_list[index - CONSTANTS.LOOKBACK_WILLIAMS + 1]: current_date, symbol].max()
		william_r_df.ix[current_date, CONSTANTS.LOWEST_LOW] = \
			prices.ix[date_list[index - CONSTANTS.LOOKBACK_WILLIAMS + 1]: current_date, symbol].min()
...
if __name__ == "__main__":
	plot_price_sma(CONSTANTS.DEFAULT_SYMBOL, CONSTANTS.IN_SAMPLE_START, CONSTANTS.IN_SAMPLE_END)
	plot_williams(CONSTANTS.DEFAULT_SYMBOL, CONSTANTS.IN_SAMPLE_START, CONSTANTS.IN_SAMPLE_END)
	plot_rsi(CONSTANTS.DEFAULT_SYMBOL, CONSTANTS.IN_SAMPLE_START, CONSTANTS.IN_SAMPLE_END)
